...
antwoord = input("Waar gaan je interesses naar uit? ")
...
hits["Het Pantheon"] = zoekop(antwoord,s)
...
hits["het Colosseum"] = zoekop(antwoord,s)
...
hits["De Trevifontein"] = zoekop(antwoord,s)
...
hits["De Sint-Pietersbasiliek"] = zoekop(antwoord,s)
...
hits["De Spaanse Trappen"] = zoekop(antwoord,s)
...
for plaats, aantal in hits.items():
    if MeesteHits < aantal:
        MeesteHits = aantal
        Bezienswaardigheid_met_de_meeste_hits = plaats
print("De beste locatie voor jou is:	" + Bezienswaardigheid_met_de_meeste_hits)
